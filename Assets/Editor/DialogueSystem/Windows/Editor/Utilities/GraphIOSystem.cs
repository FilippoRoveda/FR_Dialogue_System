...
namespace DS.Editor.Windows.Utilities
{
    ...
    public class GraphIOSystem
    {
        ...
        {
            BaseIO.CreateFolder("Assets/Editor/Files", "Graphs");
        ...
        public Action<GraphElement> FetchGraphElements()
        {
            return graphElement =>
            {
                if (graphElement.GetType() == typeof(SingleNode) ||
                    graphElement.GetType() == typeof(MultipleNode) ||
                    graphElement.GetType() == typeof(StartNode) ||
                    graphElement.GetType() == typeof(EndNode) ||
                    graphElement.GetType() == typeof(BranchNode) ||
                    graphElement.GetType() == typeof(EventNode))
                {
                    nodes.Add((BaseNode)graphElement);
                }
